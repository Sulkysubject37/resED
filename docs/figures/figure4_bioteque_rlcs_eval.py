...
def perturb_shock(z, shock_prob=0.02, factor=5.0):
    """Scale random subset of vectors."""
    rng = np.random.default_rng(42)
...
def perturb_drift(z, factor=0.5):
    """Shift mean by factor * std."""
    # Simple drift: shift all by a constant vector
    # Vector is uniform 1.0 scaled by factor
    return z + factor

def perturb_dropout(z, drop_prob=0.2):
    """Zero out random dimensions."""
    rng = np.random.default_rng(42)
    # Mask is over dimensions (columns)
    d_z = z.shape[1]
    mask = rng.random(d_z) > drop_prob # Keep prob
    z_out = z.copy()
    z_out[:, ~mask] = 0.0
    return z_out
...
def evaluate():
    ...
    # Compute Reference Statistics (from Clean)
    ref_mu = np.mean(z_clean, axis=0)
    ref_std = np.std(z_clean, axis=0)
    scalar_sigma = np.mean(ref_std)
    if scalar_sigma < 1e-6: scalar_sigma = 1.0
